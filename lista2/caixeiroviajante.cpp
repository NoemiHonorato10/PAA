...
int minimo(const std::vector<std::vector<int>> &cidades, int posicao, int visitou, std::vector<std::vector<int>> &estado)
{
    if (visitou == ((1 << cidades.size()) - 1))
        return cidades[posicao][0]; // voltar para a cidade de partida
//constantes INT_MAX = Valor máximo para um objeto do tipo int, 32767( ) ou maior *2^15-1
    if (estado[posicao][visitou] != INT_MAX)
...
int main()
{
    std::vector<std::vector<int>> cidades = {{0, 20, 42, 35},
                                            {20, 0, 30, 34},
                                            {42, 25, 0, 12},
                                            {35, 10, 12, 0}};

    std::vector<std::vector<int>> estado(cidades.size());
    for (auto &vizinhas : estado)
    //constantes INT_MAX = Valor máximo para um objeto do tipo int
        vizinhas = std::vector<int>((1 << cidades.size()) - 1, INT_MAX); 
    std::cout << "Custo minino: " << minimo(cidades, 0, 1, estado) << std::endl;
