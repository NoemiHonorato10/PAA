...
class No
{
    int valor;
    public:
    std::shared_ptr<No> esquerdaPtr;
    std::shared_ptr<No> direitaPtr;
...
int main()
{
    std::shared_ptr<No> ptr = std::make_shared<No>(4);
    ptr->esquerdaPtr = std::make_shared<No>(2);
    ptr->esquerdaPtr->paiPtr = ptr;
    ptr->direitaPtr = std::make_shared<No>(5);
    ptr->direitaPtr->paiPtr = ptr;
    std::cout<<"contagem de referencia ptr = "<<ptr.use_count()<<std::endl;
    std::cout<<"ptr-> contagem de referencia EsquerdaPtr = "<<ptr->esquerdaPtr.use_count()<<std::endl;
    std::cout<<"ptr-> contagem de referencia de DireitaPtr = "<<ptr->direitaPtr.use_count()<<std::endl;
    //lock () pode retornar shared_ptr vazio se esse shared_ptr us já foi excluído.
    std::cout<<"ptr-> direitaPtr-> contagem de referencia paiPtr = "<<ptr->direitaPtr->paiPtr.lock().use_count()<<std::endl;
    std::cout<<"ptr-> esquerdaPtr-> paiPtr contagem de referencia = "<<ptr->esquerdaPtr->paiPtr.lock().use_count()<<std::endl;
    return 0;
}
// Referências
